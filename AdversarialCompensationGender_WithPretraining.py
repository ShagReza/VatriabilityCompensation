...
input_tensor = Input(shape=(L,))
x=input_tensor
x = Dense(1000, activation="relu",name='w1')(x)
x = Dense(1000, activation="relu",name='w2')(x)
x = Dense(1000, activation="relu",name='w3')(x)
x = Dense(1000, activation="relu",name='w4')(x)
x = Dense(1000, activation="relu",name='w5')(x)
flip_layer = GradientReversal(GRLstrength)
y= flip_layer(x)
y= Dense(units=2, activation='softmax', name='domain_classifier')(y)
...
#------------------------------------------------------------------------------
history = model.fit(x_train, [y_train,G_train], 
                    batch_size=128, 
                    epochs=40,                    
                    shuffle=True,
                    verbose=1,
...
model.save('model_GenderCompensation.h5')
#------------------------------------------------------------------------------
...
NumOutput=2
TsneSpk(intermediate_output_pca,GenderI,NumOutput)
# find the embedings of model (with adversarial)
# draw them
intermediate_layer_model = Model(inputs=model.input,outputs=model.get_layer('w5').output)
intermediate_output = intermediate_layer_model.predict([LandmrakI])
pca = PCA(n_components=50)
pca.fit(intermediate_output) 
intermediate_output_pca=pca.transform(intermediate_output)

...
z=y5
#
model=Model([input_1,input_y2], [y2_forward,z])
model.summary()
#-----------------------------------------------------------------------------
# train:
opt1 = optimizers.Adam(lr=0.00002, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0)
model.compile(loss={'v2': 'mean_squared_error','v5a': 'mean_squared_error'},
              loss_weights={'v2': 0.2,  'v5a': 0.6},optimizer=opt1)
...
for i in range(NumParts):
    ...
    y2forward=model.predict([x_train,y2before])[0]
    y2before=np.tanh(y2forward)
    y2feedback=Feedback2.predict([x_train,y2before,])
    y2=np.tanh((1-landa)*y2forward+y2feedback)
    y2feedback=Feedback2.predict([x_train,y2])
...
for j in range(100):     
    ...
    for i in range(NumParts):
        ...
        #
        history = model.fit([x_train,y2before], [y2new,y_train],epochs=1,shuffle=True,callbacks= [reduce_LR])
        #
        y2forward=model.predict([x_train,y2before])[0]
        y2feedback=Feedback2.predict([x_train,y2before])
        y2=np.tanh((1-landa)*y2forward+y2feedback)

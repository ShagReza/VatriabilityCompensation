...
x = Dense(1000, activation="relu",name='v0')(x)
...
y = Dense(1000, activation="relu",name='v0')(y)
...
for i in range(1):
    ...
    WeightL7 = model1.layers[7].get_weights()
    model.layers[7].set_weights(WeightL7)
    ...
    WeightL7 = model.layers[7].get_weights()
    model1.layers[7].set_weights(WeightL7)
    history = model1.fit(x_train, y_train, batch_size=128,  epochs=1, shuffle=True,verbose=1)
#------------------------------------------------------------------------------
